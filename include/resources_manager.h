...
namespace skibidi
{
    ...
    public:
        ...
        Texture2D& getTexture(const std::string& file)
        {
            ...
            {
                std::string msg = "No se encontro textura: " + fullPath;
        ...
        Font& getFont(const std::string& file)
        {
            ...
            if (!FileExists(fullPath.c_str()))
            {
                std::string msg = "No se encontro fuente: " + fullPath;
            ...
            fonts[fullPath] =LoadFontEx(fullPath.c_str(), 16, nullptr, 0);
            return fonts[fullPath];
        }
        ...
        Sound& getSound(const std::string& file)
        {
            ...
            if (!FileExists(fullPath.c_str()))
            {
                std::string msg = "No se encontro sonido: " + fullPath;
                TraceLog(LOG_WARNING, "%s", msg.c_str());
        ...
        Music& getMusic(const std::string& file)
        {
            ...
            if (!FileExists(fullPath.c_str()))
            {
                std::string msg = "No se encontro musica: " + fullPath;
            ...
            music[fullPath] = LoadMusicStream( fullPath.c_str());
            return music[fullPath];
        }
